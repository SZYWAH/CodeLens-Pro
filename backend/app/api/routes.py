...
from backend.app.services.llm_settings_service import (
    clear_user_deepseek_key,
    effective_deepseek_base_url,
    effective_deepseek_key,
    llm_key_status,
    save_user_deepseek_key,
    test_deepseek_key,
)
...
def health() -> HealthResponse:
    ...
    return HealthResponse(
        ...
        llm_key_configured=bool(effective_deepseek_key()),
    )
...
def read_llm_key_status() -> LLMKeyStatusResponse:
    return LLMKeyStatusResponse(**llm_key_status())


@router.post("/llm/key", response_model=LLMKeyTestResponse)
def save_llm_key(payload: LLMKeySaveRequest) -> LLMKeyTestResponse:
    result = save_user_deepseek_key(payload.api_key)
...
@router.delete("/llm/key", response_model=LLMKeyStatusResponse)
def delete_llm_key() -> LLMKeyStatusResponse:
    return LLMKeyStatusResponse(**clear_user_deepseek_key())


@router.post("/llm/key/test", response_model=LLMKeyTestResponse)
def test_llm_key(payload: LLMKeyTestRequest) -> LLMKeyTestResponse:
    result = test_deepseek_key(payload.api_key)
    return LLMKeyTestResponse(
        ok=bool(result.get("ok")),
        status=str(result.get("status") or ""),
        detail=str(result.get("detail") or ""),
...
def _llm_error_payload(exc: Exception) -> dict[str, str]:
    ...
    if "DEEPSEEK_API_KEY" in message:
        return error_payload(
            "LLM_KEY_MISSING",
            message,
            "请在设置页填写 DeepSeek 官方 API Key，或在 .env 中配置 DEEPSEEK_API_KEY 后重启后端。",
        )
    return error_payload(
        "LLM_REQUEST_FAILED",
        message,
        "请检查模型名称、网络连接、DeepSeek 余额和官方接口连接状态。",
    )
