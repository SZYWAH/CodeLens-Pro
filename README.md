...
- **代码工作台**：粘贴代码或接收 VS Code 插件传入的当前文件/选中代码，执行静态分析，生成 SSE 流式 AI 报告。
- **代码对比**：对两个版本代码生成差异分析、风险提示、性能对比和改进建议。
- **报告阅读器**：支持全屏阅读、PyCharm 风格代码块、复制代码、报告上下文追问和多轮对话定位。
- **AI 对话**：普通聊天独立保存，支持历史会话搜索、继续对话和上下文无关的编程问答。
- **Agent 工作区**：通过 VS Code 插件同步当前项目、项目树和心跳状态；支持手动选择文件、AI 自动选择上下文、混合补充上下文，并生成可确认/执行的修改计划。
- **项目导读**：基于 VS Code 插件同步的项目结构生成导读建议，帮助快速理解项目目录、入口和可能涉及的知识点。
- **知识卡片**：从报告、对话或手动输入沉淀知识点；支持详情页、学习资料生成、代码摘录、外部资源、状态管理和个人笔记。
- **每日日志**：汇总当天报告、AI 对话、知识卡片、Agent 使用情况，调用 LLM 生成日志；无使用记录的日期也可以手写日记。
- **历史报告**：报告自动命名、搜索、筛选、查看详情、删除，并可回到工作台或代码对比页面继续使用。
- **统计看板**：展示 API 余额、Token、报告数、会话数、最近活动和功能使用情况。
- **活动星图**：从统计页进入全屏 3D 星图，每颗主星代表报告、聊天或 Agent 活动，支持拖拽旋转、滚轮缩放和点击查看详情。
...
7. 后端生成 Agent 计划后，插件端读取选中文件内容、执行或回传结果，Web 端展示计划、状态和上下文文件 chips。
...
插件版使用独立的 `vscode-extension/webview-mini/` 前端，视觉风格跟随 VS Code 原生主题，不复用 Web 版的大导航、统计页和复杂分栏。Web 版 Agent 页面中的项目树和上下文选择能力则依赖插件端的心跳与任务执行服务。
