...
1. 克隆或下载本项目到本地。
2. 根据 `.env.example` 配置数据库；DeepSeek 官方 API Key 可以在 `.env` 中配置，也可以启动后在设置页填写。
...
- **Agent 工作区**：协作主线入口。通过 VS Code 插件同步当前项目、项目树和心跳状态；讨论与修改计划合并为一条工作流，支持 `自动判断 / 仅讨论 / 生成修改计划`，并在用户确认后交由插件执行。
- **项目导读**：协作主线的辅助视角。已从侧边栏一级入口降级到 Agent 工作区的“当前项目”面板中；点击“项目导读”按钮后弹出导读浮层，展示项目架构、文件说明、核心区域和推荐阅读顺序。
- **知识卡片**：学习沉淀主线。从报告、对话或手动输入沉淀知识点；支持详情页、学习资料生成、代码摘录、外部资源、状态管理和个人笔记；“从未沉淀报告智能提炼”会优先处理最近未保存过知识卡片的报告，减少重复提炼。
...
- **统计看板**：复盘展示层。展示 API 余额、Token 总量、报告数、会话数、最近活动和功能使用情况，并提供 DeepSeek 官方 API Key 的保存、测试和清除入口。
- **活动星图**：复盘展示层。从统计页进入全屏 3D 星图，每颗主星代表报告、聊天或 Agent 活动，支持拖拽旋转、滚轮缩放和点击查看详情。
- **本地持久化**：报告、静态指标、聊天会话、Agent 计划、学习卡片、每日工作日志等保存到本机 MySQL。

## 当前产品边界

- Web 端不会直接读取本地项目文件内容；Agent 对话和修改计划需要文件内容时，会创建临时上下文读取请求，由 VS Code 插件读取后回传。
...
- DeepSeek Base URL 固定为官方地址 `https://api.deepseek.com/v1`；当前版本只支持填写 DeepSeek 官方 Key，不开放自定义 Base URL 或第三方模型自动检测。
- 有效 Key 优先级为：页面保存的用户 Key > `.env` 中的 `DEEPSEEK_API_KEY`。接口返回只展示脱敏 Key，不返回完整 Key。
...
7. Agent 输入框支持自动判断意图：普通问题会生成讨论回复；“帮我落实 / 按上面改 / 生成计划”等任务会结合聊天历史和已选文件生成修改计划。
8. Web 端确认计划后，插件端读取或修改工作区文件，并向后端回传执行结果；Web 端展示计划、状态、上下文文件 chips 和执行结果。
9. 用户可将重要知识沉淀为知识卡片，生成学习资料、记录笔记并标记复习状态。
10. 每日日志按日期汇总报告、对话、Agent 和知识卡片活动；有活动可生成日志，无活动也可作为个人日记本手写保存。
11. 统计页汇总使用数据，并可进入活动星图查看报告、聊天和 Agent 活动的空间化呈现。
...
也可以不在 `.env` 中填写 `DEEPSEEK_API_KEY`，启动系统后进入“统计看板”中的 **DeepSeek API Key** 面板保存官方 Key。页面保存的 Key 会优先于 `.env` 生效，保存、测试、清除后 TopBar、统计看板、报告生成、AI 对话、知识卡片、每日工作日志和 Agent 都会使用同一份有效 Key。

当前版本为了保证课堂 Demo 稳定，只支持 DeepSeek 官方 Key，Base URL 固定为 `https://api.deepseek.com/v1`，不提供第三方 Base URL 输入框。
...
4. 可选：在统计看板填写 DeepSeek 官方 API Key，或在 `.env` 中配置 `DEEPSEEK_API_KEY` 后，可测试 AI 报告生成
...
- `POST /api/agent/message/stream`：Agent 统一消息入口，自动判断是讨论还是生成计划。
- `POST /api/agent/chat/stream`：Agent 讨论流式返回，必要时会请求插件读取所选文件内容。
...
- `GET /api/agent/chat-context/pending`：插件端拉取待读取上下文请求。
- `POST /api/agent/chat-context/{request_id}/result`：插件端回传文件读取结果。
...
- `GET /api/learning/project-guide`：读取项目导读数据，返回“项目架构 + 文件说明”的静态阅读视图。
- `GET /api/llm/key`：读取当前 DeepSeek Key 配置状态。
- `POST /api/llm/key`：保存 DeepSeek 官方 API Key。
- `DELETE /api/llm/key`：清除页面保存的 Key。
- `POST /api/llm/key/test`：测试当前输入或已保存的 Key。
...
│   │   │   ├── llm_settings_service.py # 页面保存的 DeepSeek Key 配置
...
│       │   ├── ProjectGuidePage.tsx    # 项目导读弹窗/隐藏路由复用内容
...
静态分析、历史、统计页面仍可使用，但报告生成和 AI 对话会返回友好错误。可以填写 `DEEPSEEK_API_KEY`，也可以在统计看板里保存 DeepSeek 官方 Key。页面保存的 Key 会优先于 `.env`。
...
Agent 的“当前项目”面板依赖 VS Code 插件每 5 秒上报一次 workspace heartbeat。后端把 60 秒内未更新视为可用，20-60 秒会显示同步延迟，超过 60 秒才显示“插件未连接”。排查顺序：
...
5. 如果超过 60 秒没有心跳，Web 端才会显示“插件未连接或状态已过期”。
...
三种模式都不会让浏览器直接读取本地文件内容；最终读取发生在 VS Code 插件端，并会校验路径不能越出 workspace root。当前 Agent 输入框还支持“自动判断”意图，普通讨论和生成修改计划会自动分流到不同工作流。

### 为什么知识卡片按钮叫“从未沉淀报告智能提炼”

这个按钮默认只处理最近还没有保存过知识卡片的报告，避免把已经在报告生成时沉淀过的内容再提炼一遍。这样更省时间，也更少出现重复候选。
...
Web 端 Agent 页面
  ...
  ├─ 勾选文件或选择上下文模式
  ├─ 点击当前项目面板中的“项目导读”打开项目架构说明
  ├─ POST /api/agent/message/stream 统一处理讨论和计划生成
  └─ 确认计划后等待插件执行
```

项目树默认排除 `.git`、`node_modules`、`.venv`、`dist`、`build`、`__pycache__` 等目录，并限制深度和节点数量，避免过量同步。项目导读弹窗会尽量直接读取本地项目结构和少量 UTF-8 文本内容，为目录和文件补充一句说明；遇到二进制、大文件或无权限文件时会回退为通用说明。
