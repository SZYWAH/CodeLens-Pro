...
exports.getActiveEditorContext = getActiveEditorContext;
exports.getFileContext = getFileContext;
exports.pickFileContexts = pickFileContexts;
exports.getOpenTextFileContexts = getOpenTextFileContexts;
exports.pickProjectFileContexts = pickProjectFileContexts;
exports.readWorkspaceSelectedFileContexts = readWorkspaceSelectedFileContexts;
exports.collectWorkspaceManifest = collectWorkspaceManifest;
exports.collectWorkspaceRuleContexts = collectWorkspaceRuleContexts;
...
async function getFileContext(uri) {
...
async function getOpenTextFileContexts(limit = 5) {
    const candidates = [];
...
async function pickProjectFileContexts() {
    ...
    const selected = await vscode.window.showQuickPick(candidates.map((uri) => ({
...
async function collectWorkspaceRuleContexts() {
    ...
    if (!choice)
        return [];
    if (choice.rule === "recent")
        return getOpenTextFileContexts(10);
    let uris = [];
    if (choice.rule === "currentDir") {
        ...
        uris = await vscode.workspace.findFiles("{**/package.json,**/tsconfig*.json,**/vite.config.*,**/pyproject.toml,**/requirements*.txt,**/.env.example,**/*.yaml,**/*.yml,**/*.ini}", WORKSPACE_EXCLUDE, 80);
    }
    const contexts = await readContextBundle(await filterReadableUris(uris));
    if (!contexts.length)
        vscode.window.showWarningMessage("该规则没有收集到可用于分析的文件。");
    return contexts;
}
async function autoCollectWorkspaceContexts() {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
...
async function filterReadableUris(uris) {
    const result = [];
    const seen = new Set();
...
function scoreAutoContextPath(relativePath) {
    const normalized = relativePath.replace(/\\/g, "/");
    const fileName = path.basename(normalized).toLowerCase();
    let score = 0;
    if (/^readme/.test(fileName))
        score += 100;
    if (["package.json", "requirements.txt", "pyproject.toml", "alembic.ini", ".env.example", "dockerfile"].includes(fileName))
        score += 90;
    if (normalized === "backend/app/main.py")
        score += 85;
    if (normalized === "frontend/src/App.tsx")
        score += 80;
    if (normalized === "vscode-extension/webview-mini/src/App.tsx")
        score += 80;
    ...
    if (normalized.startsWith("frontend/src/"))
        score += 35;
    if (normalized.startsWith("vscode-extension/src/"))
        score += 30;
    if (normalized.startsWith("vscode-extension/webview-mini/src/"))
        score += 30;
    return score;
}
...
function buildMultiFileContext(files, sourceType) {
    const labels = {
        pickedFiles: "本地文件",
        recentFiles: "最近文件",
        workspaceFiles: "项目文件",
