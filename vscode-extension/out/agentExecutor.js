...
class AgentWorkspaceExecutor {
    ...
    async handlePendingTask(task) {
        ...
        await this.reportTaskProgress(task, "context_resolve", contextMode === "manual"
            ? `正在读取网页选中的 ${selectedPaths.length} 个上下文文件...`
            : "正在解析项目上下文范围，准备读取相关文件...", undefined, selectedPaths).catch(() => undefined);
        ...
        if (effectiveContextPaths.length) {
            await this.reportTaskProgress(task, "read_selected_files", `正在读取 ${effectiveContextPaths.length} 个工作区文件...`, undefined, effectiveContextPaths).catch(() => undefined);
        }
        else {
        ...
        await this.reportTaskProgress(task, "context_ready", `已读取 ${files.length} 个上下文文件，正在打包发送给后端生成计划...`, undefined, actualPaths).catch(() => undefined);
    ...
    async resolveContextPaths(task, contextMode, selectedPaths, progressPlan) {
    ...
    async applyConfirmedPlan(plan) {
        ...
        for (let index = 0; index < operations.length; index += 1) {
            ...
            await this.reportPlanResult(plan, "confirmed", `正在准备第 ${index + 1}/${operations.length} 个操作：${operation.type} ${operation.path}`).catch(() => undefined);
            if (!this.isInsideWorkspace(rootPath, targetPath)) {
                skipped.push(operation.path);
                await this.reportPlanResult(plan, "confirmed", `已跳过工作区外路径：${operation.path}`).catch(() => undefined);
                continue;
            }
            ...
            await this.reportPlanResult(plan, "confirmed", `已准备第 ${index + 1}/${operations.length} 个操作：${operation.type} ${operation.path}`).catch(() => undefined);
        }
        try {
    ...
    async reportTaskProgress(plan, phase, message, detail, selectedFilePaths = []) {
        const planId = plan.plan_id || plan.id;
        if (!planId)
            return;
    ...
    async reportFileReadProgress(plan, event) {
        const fileName = path.basename(event.path);
        if (event.status === "reading") {
            await this.reportTaskProgress(plan, "file_reading", `正在读取第 ${event.index}/${event.total} 个文件：${fileName}`, event.path, [event.path]).catch(() => undefined);
            return;
        }
        if (event.status === "read") {
            await this.reportTaskProgress(plan, "file_read", `已完整读取第 ${event.index}/${event.total} 个文件：${fileName}`, `${event.path}${event.chars !== undefined ? ` · ${event.chars} 字符` : ""}`, [event.path]).catch(() => undefined);
            return;
        }
        if (event.status === "summarized") {
            await this.reportTaskProgress(plan, "file_summarized", `已读取并提取第 ${event.index}/${event.total} 个大文件的关键点：${fileName}`, `${event.path}${event.reason ? ` · ${event.reason}` : ""}`, [event.path]).catch(() => undefined);
            return;
        }
        await this.reportTaskProgress(plan, "file_skipped", `已跳过第 ${event.index}/${event.total} 个文件：${fileName}`, `${event.path}${event.reason ? ` · ${event.reason}` : ""}`, []).catch(() => undefined);
    }
