...
export function AgentPage({
  ...
  const workspaceReady = workspaceState === "ready" || workspaceState === "delayed";
          ...
          <div className="border-b border-line p-3">
            ...
            >
              ...
              >
                ...
                <span className={["agent-workspace-dot", workspaceState === "ready" || workspaceState === "delayed" ? "agent-workspace-dot-live" : ""].join(" ")} />
              </button>
              {workspacePanelOpen ? (
...
function getWorkspaceConnectionState(snapshot: WorkspaceSnapshot | null): "ready" | "delayed" | "blocked" | "waiting" {
  if (!snapshot?.updated_at || !snapshot.connected) return "waiting";
  if (getWorkspaceHeartbeatAgeSeconds(snapshot) > WORKSPACE_HEARTBEAT_STALE_SECONDS) return "waiting";
  if (snapshot.status === "no_workspace") return "blocked";
  if (getWorkspaceHeartbeatAgeSeconds(snapshot) > WORKSPACE_HEARTBEAT_DELAYED_SECONDS) return "delayed";
  return "ready";
}

function workspaceStatusLabel(snapshot: WorkspaceSnapshot | null) {
  const state = getWorkspaceConnectionState(snapshot);
  if (!snapshot) return "正在读取心跳";
  if (state === "waiting") return "未收到插件心跳";
  if (state === "delayed") return "同步延迟";
...
function workspaceDiagnosticReason(snapshot: WorkspaceSnapshot | null) {
  ...
  if (state === "ready" && snapshot.status === "connected") return "连接正常";
  if (state === "delayed") return "最近一次心跳超过 20 秒，正在等待插件同步。";
  ...
  if (state === "waiting") return "心跳超过 60 秒未更新，请确认插件连接的后端端口一致";
